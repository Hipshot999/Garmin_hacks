...
printToFile = True
fileName = 'C:\\Pelle\\Python Garmin hack\\datafile.txt'
# If dry_run parameter, print json, otherwise print to Idle.

printJson = True
# If True will write the whole Json, otherwise write selected items.
...
if printJson:
    for a in JSON_LIST:
        ...
        if a["activityType"]["typeKey"] == "kayaking":
            if printToFile:
                dataFile.write(str(a["activityId"]) + '|' + a["activityType"]["typeKey"] + '|' + a["activityName"])
                print('Writing to file activity ID: ' + a["activityName"] + ' activityName: ' + a["activityName"])
#                json.dump(a,dataFile)
                dataFile.write('\n')
            else:
                print(a)
else:
    for a in JSON_LIST:
        print(f'{a["activityId"]}| {a["startTimeLocal"]}|{a["activityName"] if a["activityName"] else ""}|{a["activityType"]["typeKey"]}|(typeId: {a["activityType"]["typeId"]}, parentTypeId: {a["activityType"]["parentTypeId"]}|description: {a["description"]})')
        if printToFile:
            dataFile.write(f'{a["activityId"]}| {a["startTimeLocal"]}|{a["activityName"] if a["activityName"] else ""}|{a["activityType"]["typeKey"]}|(typeId: {a["activityType"]["typeId"]}, parentTypeId: {a["activityType"]["parentTypeId"]}|description: {a["description"]})')
